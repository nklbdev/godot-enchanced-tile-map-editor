...
func _init(selection_paper: Selection, tiles_paper: Paper, autotiles_paper: Paper, terrains_paper: Paper) -> void:
	...
	var __eraser_pattern_holder: Common.ValueHolder = Common.ValueHolder.new(Patterns.Pattern.from_rect_and_data(Rect2(Vector2.ZERO, Vector2.ONE), [-2, 0, 0, 0], TileMap.HALF_OFFSET_DISABLED))
