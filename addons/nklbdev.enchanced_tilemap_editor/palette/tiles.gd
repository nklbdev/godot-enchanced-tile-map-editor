...
var __paint_pattern_layout_map: PatternLayoutMap
var __selection_pattern_layout_map: PatternLayoutMap
...
func _init(selection_paper: Selection, tiles_paper: Paper, eraser: Instrument).("Tiles", "tiles", [
	...
	__paint_pattern_layout_map = PatternLayoutMap.new(tiles_paper)
	__selection_pattern_layout_map = PatternLayoutMap.new(tiles_paper)
	__selection_pattern_layout_map.pattern = Patterns.Pattern.new(Vector2.ONE, [0, 0, 0, 0])
	...
	var instrument_brush: InstrumentStamp = InstrumentStamp.new(tiles_paper, __paint_pattern_layout_map, selection_map)
	var instrument_line: InstrumentLine = InstrumentLine.new(tiles_paper, __paint_pattern_layout_map, selection_map)
	var instrument_rectangle: InstrumentRectangle = InstrumentRectangle.new(tiles_paper, __paint_pattern_layout_map, selection_map)
	var instrument_bucket_fill: InstrumentFlood = InstrumentFlood.new(tiles_paper, __paint_pattern_layout_map, tiles_paper, selection_map)
	...
	tb.node(toolbar).with_children([
		tb.node(toolbar.create_instrument_button("Rectangle Selection", KEY_M, "rectangle_selection", InstrumentRectangle.new(selection_paper, __selection_pattern_layout_map, null, false))),
		tb.node(toolbar.create_instrument_button("Same Tile Selection", KEY_W, "magic_wand", InstrumentFlood.new(selection_paper, __selection_pattern_layout_map, tiles_paper, null))),
...
func __update_transform_menu(menu: PopupMenu) -> void:
	var half_offset_type: Common.CellHalfOffsetType = Common.CELL_HALF_OFFSET_TYPES[__paint_pattern_layout_map.cell_half_offset]
...
func __on_transform_popup_menu_item_pressed(item_index: int, popup_menu: PopupMenu) -> void:
	var pattern: Patterns.Pattern = __paint_pattern_layout_map.pattern
	if pattern:
		var meta = popup_menu.get_item_metadata(item_index)
		if meta:
			if "flipping" in meta:
				pattern.flip(meta.flipping, __paint_pattern_layout_map.cell_half_offset)
			if "rotation" in meta:
				pattern.rotate_ccw(meta.rotation, __paint_pattern_layout_map.cell_half_offset)
			if "cell_transform" in meta:
				pattern.transform_cells(meta.cell_transform)

func _on_subpalette_selected(data: Patterns.Pattern) -> void:
	__paint_pattern_layout_map.pattern = data
...
func __on_selection_pattern_copied(pattern: Patterns.Pattern) -> void:
	_on_subpalette_selected(pattern)
