...
#func get_origin_map_cell(world_pos: Vector2, cell_half_offset_type: int) -> Vector2:
#	var origin: Vector2 = world_pos - size / 2
#	return (origin if cell_half_offset_type == TileMap.HALF_OFFSET_DISABLED else
#		# TODO подточить, чтобы мышь была всегда в красном прямоугольнике
#		(Vector2(origin.x + 0.25 * sign(cell_half_offset_type - 2) - 0.5 * (int(origin.y) & 1), origin.y)
#		if cell_half_offset_type == TileMap.HALF_OFFSET_X or cell_half_offset_type == TileMap.HALF_OFFSET_NEGATIVE_X else
#		Vector2(origin.x, origin.y + 0.25 * sign(cell_half_offset_type - 2) - 0.5 * (int(origin.x) & 1)))).round()
