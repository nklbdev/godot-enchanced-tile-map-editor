...
func _after_pushed() -> void:
	__line.append(_pattern_layout_map.world_to_map(_position) if _pattern_layout_map.pattern_size == Vector2.ONE else Vector2.ZERO)
	paint()
	...
	if _is_pushed:
		if _pattern_layout_map.pattern_size == Vector2.ONE:
			...
			if position_map_cell != previous_position_map_cell:
				__line = Algorithms.get_line(previous_position_map_cell, position_map_cell, _pattern_layout_map.cell_half_offset)
				paint()
		else:
